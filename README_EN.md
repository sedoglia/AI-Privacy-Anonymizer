...
### Layer 1 — OpenAI Privacy Filter (OPF, recommended)

For the most complete PII coverage, also install **OPF**, the first layer of the pipeline (context-aware semantic detection). It is not included in the `[recommended]` extra because it is distributed on GitHub only:

```bash
pip install "opf @ git+https://github.com/openai/privacy-filter"
```

> Requires ~3 GB of disk space for the model download on first run. Alternatively, run `privacy-anonymizer --install-full` to install both `[recommended]` and OPF in a single command.
...
- Configurable threshold (default: 0.3) to balance recall and precision
...
| PDF (selectable or image) | `.pdf` | `pypdf` + RapidOCR (ONNX) | PyMuPDF overlay / OCR redaction | If selectable text is ≥ 20 characters: coordinate redaction. Below threshold (e.g. only "Page 1 of 1"): automatic OCR fallback as for scanned PDFs |
...
| JSON | `.json` | built-in | built-in | String leaf values; structure, numbers, and booleans preserved |
...
Self-contained `.exe` for Windows — no Python or any dependency required for the end user.

### Pre-built download (recommended)

**[⬇ Download privacy-anonymizer.exe (Google Drive, ~2.68 GB)](https://drive.google.com/file/d/1B0UpI_RmFGstKsH1ufU8GbownWz4qy7u/view?usp=sharing)**

> The file (~2.68 GB) is hosted on Google Drive because it exceeds GitHub's 2 GB limit.

After downloading, run it from the terminal:

```
privacy-anonymizer.exe document.pdf
```

> On first run, ML models (~500 MB) are downloaded automatically.

### Build from source

To regenerate the executable locally:

```bash
pyinstaller dist/privacy_anonymizer.spec --distpath dist --workpath build/pyinstaller
```

Output: `dist/privacy-anonymizer.exe` (~2.68 GB). The bundle includes all three ML layers (OPF, GLiNER, patterns), RapidOCR, all format adapters, and the Python runtime.

Build files are version-controlled in `dist/`:

| File | Purpose |
|---|---|
| `dist/privacy_anonymizer.spec` | PyInstaller spec (hiddenimports, runtime hooks) |
| `dist/hooks/rthook_tiktoken.py` | Runtime hook: fixes a tiktoken bug in PyInstaller bundles (`tiktoken_ext` namespace package not found by `pkgutil.iter_modules`) |

> **Note:** `dist/privacy-anonymizer.exe` is excluded from the repository (too large). Regenerate it locally with the command above.
...
The full hybrid stack (OPF + GLiNER + patterns) is active by default. To reduce layers:
...
# Italian patterns only (fastest, no ML dependencies)
...
# Balanced mode (balanced for chatbot use)
privacy-anonymizer file.txt --recall-mode balanced

# Aggressive mode (default — maximum recall)
...
```bash
# Confidence threshold (default: 0.3 — higher values increase precision, reduce recall)
...
# ML device: auto (default, picks CUDA/MPS if available), cpu, cuda, mps
...
|------|--------|---------------|
| `--device auto` (default) | Picks CUDA → MPS → CPU based on availability | Always, unless you need to force one |
| `--ocr-dpi 200` | Lowers OCR render DPI from 300 → 200 (~50% faster, slight quality loss) | Scanned PDFs with large/clear text |
| `--no-ocr-parallel-pages` | Disables per-page parallel OCR | If the machine is RAM-limited |
| `--ocr-max-workers N` | Max threads for parallel page OCR (default 4) | Match your CPU core count |
| `--no-chunk-long-text` | Disables parallel chunking of long texts for ML layers | Debugging or texts <4 KB |
| `--chunk-threshold N` | Minimum chars to enable chunking (default 4000) | Raise to avoid chunking on medium texts |
| `--chunk-size N` | Window length in characters (default 1500) | Lower if model truncates, raise for more context |
| `--chunk-overlap N` | Overlap between windows (default 100) | Raise if you see entities cut at boundaries |
| `--chunk-max-workers N` | Threads to process chunks in parallel (default 4) | Match your CPU core count |
| `--ml-skip-extensions ".log,.txt"` | Extensions for which ML layers (GLiNER/OPF) are skipped above `--ml-skip-min-chars` | Log/dump files where patterns alone suffice |
| `--ml-skip-min-chars N` | Char threshold above which ML skip kicks in (default 8000) | Smaller files still go through the ML layers |

```bash
# Faster OCR on scanned PDFs: 200 DPI + 8 threads for parallel page processing
privacy-anonymizer scan.pdf --ocr-dpi 200 --ocr-max-workers 8

# Long log files: patterns only, skip GLiNER/OPF (dramatically faster)
privacy-anonymizer application.log --ml-skip-extensions ".log,.txt" --ml-skip-min-chars 4000

# Disable chunking if you want GLiNER/OPF to see the whole text
privacy-anonymizer document.txt --no-chunk-long-text
```
...
By default, all informational messages from external libraries (RapidOCR, transformers, etc.) are suppressed to keep the output clean. Using `--log` enables verbose logging mode: all messages (INFO, DEBUG, WARNING) are written to a file, including those from external libraries.

```bash
# Verbose log with auto-generated filename (privacy_anonymizer_YYYYMMDD_HHMMSS.log)
privacy-anonymizer document.pdf --log

# Verbose log to a specified file
privacy-anonymizer ./folder/ --output ./out/ --log /tmp/debug.log
```

The log file includes timestamp, logger name, source file, and line number for every message. Console behavior remains unchanged (essential output only).
